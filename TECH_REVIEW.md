...
## 批BY：写路径砍全租牌RR兜底（Radar #33 根因，2026-09-11）
...
## 批CH：落地页批1——模板转对外技术规范+严校验拦死 + 页卡今日/7天（2026-09-14）

### 概述
用户方向：①「落地页模板」转对外技术规范（讲怎么写不讲内部实现），上传严校验符合 Tova 才能传（曾缺守卫/硬编码像素/写死链接/资源文件全 warning 放行=传上来就废）；②链接管理 UI 重构第一步。拍板：全部拦死/规范中英双语/两批交付。

### 变更表
| 项 | 文件 | 变更 | 验证 |
|---|---|---|---|
| 严校验 | `landing_lib.py`+`i18n.py` | 4 项 warning→400 error（报错引用规范条目名+资源文件名）；抽纯函数 `_validate_lp_spec`；TT 缺占位符保留 warning | smoke 9 断言（合规过/4 类违规各拒/多重合并 4 条/占位符不误伤/英文文案） |
| 规范文档 | `Landing.vue`+`landing.js` | 模板抽屉→「页面规范与自定义页面上传」三区：规范 7 节双语（单文件自包含/占位符表/守卫代码块可复制/禁硬编码像素/禁写死链接/打包规则/自检清单——零内部实现细节）+上传+已传列表 | build ✓；抽屉 600px |
| 页卡双指标 | `landing.py`+`Landing.vue` | `/landing/pages` 批量聚合附北京业务日 today/last7d 访问·通过（同 GROUP BY+CASE 桶，爬虫过滤同口径）；卡片今日主显+「7天 N · 累计 M」副行 | smoke 3 断言（端点新字段+独立 SQL 对账 today/7d 一致） |
| 设计系统对齐 | `Landing.vue` | 模式筛选→全局 list-bar/seg-bar（带计数）；页卡 hover 手感对齐 card-base | build ✓ |

### DB 迁移
- 无。

### 生产环境变更
- restart ×1（3 后端文件双门+health）；CF master；smoke 12/12。

### 复审结论（已知限制/风险）
- 存量模板不受影响（严校验只拦新上传；存量里有 warning 级问题的模板继续可用，逐步替换）。
- 「写死外链=error」含隐私政策链接——规范明确 LP 自包含；交付方需把隐私内容做进页面。
- today/last7d 与日志页同用北京业务日口径；单页兜底路径（stats=None）不填新字段（仅列表批量路径有——前端列表必经批量路径）。
- 批2 待做（已锁范围）：日志页重构（筛选分组/聚合端点/行展开/导出）+ 域名管理重构（usage 匹配修复/域名级统计）。
...
## 批CL：两天全量复审（用户强制要求，2026-09-15）

### 复审范围
09-14 00:00 → 09-15 终：约 30 commits（落地页批1/批2、三页重构、哨兵迁移、CORS、全库扫查批等）。

### 复审手段与结论
| 层 | 手段 | 结果 |
|---|---|---|
| 后端运行时 | `_audit_endpoints.py` 13 端点直跑（带认证+字段断言） | **13/13 PASS** |
| 后端异常 | journal 26h Traceback=126 条，最后一条 18:27（重启 18:50 前）= **当前代码零活跃异常**；126 条全部归属已修复的三个历史 bug（聚合列重复/优先级/未初始化） | ✓ |
| 前端 i18n | 近两天新增 key 27 项定向核对（views ×2 zh/en 成对 + 主文件 zh/en 各 1） | **全在** |
| 构建 | npm run build ✓ 且抽验 dist 产物含修复后的 `limit:K.value` | ✓ |

### 本批新发现并修复（2a44012）
**上笔扫查修复未落盘**：_fix_sweep.py 锚点补丁吞掉 `wr(p,s)` 行 → LandingLogs limit 修复只存在内存（运行输出缺 "1 LandingLogs OK" 未被核对）→ 用户继续报 422。本笔已真正写入+构建产物验证。

### 教训（并入 endpoint-verify-rule 记忆）
1. 批量脚本运行后**必须核对每步 print 清单与预期步骤数一致**（本次正是缺一步未察觉）
2. "build 绿"不构成修复证据——零改动也绿；修复验证必须 grep 源文件 + 构建产物双确认
3. 两天三个 P0 全部同族：高速批量文本编辑 + 验证停在语法层。SOP 已固化：改端点直跑、改前端 grep 产物、journal 查 traceback
...
## 批CS：消耗报告窗口严格化（2026-09-17 深夜，ab501ba）

### 概述
用户反馈「复制消耗账户并不严格遵循选定的时间窗口按钮」。根因：`_db_row` 在窗口无数据时回退 **db_last（旧日期的消耗）冒充窗口值**——用户点「今日」却看到 09-11 的钱。借机完成展现重设计（用户授权自行调研定夺）。

### 语义定稿
| 项 | 定稿 |
|---|---|
| 窗口基准 | 各账户本地日（与看板/快照同基准，useDateRange 北京日字符串→FB time_range 账户本地解释）；弹窗 meta 明示+悬浮解释 |
| 消耗列 | **严格窗口内**：在管=FB 实时 / 已移除·令牌失败=库内窗口合计 / 无记录=0.00（不再拿旧数据冒充） |
| 日期列 | 恒=窗口标签（每行自描述，TSV 粘贴可用） |
| 最后记录 | 已移除账户专属参考字段 `last_record{date,spend}`——副列+悬浮说明，不进复制 |
| 过滤 | 「仅看窗口内有消耗」开关（默认开）——表格与复制同口径 |
| 复制 | 保持 5 列 TSV：日期/归属人/账户名/账户ID/消耗金额(USD) |

### 验证（生产数据断言）
53 行 date 全=窗口标签 ✓；last_record 仅出现在已移除行（24 条）✓；零消耗行 45 全为数值 0 ✓；live=11、今日有消耗 8 账户（Alexistr603 $103.35 / PW-USD $67.67 / BSCH-TD-O349 $56.29 与 FB insights 对得上）✓；线上 chunk 含 srOnlySpend ✓。
...
## 批DB：设置页整体 UI/UX 优化（2026-09-19，本 commit）

### 方法：实测驱动
Playwright 逐区量化（标签宽/输入宽/描述长度/溢出/JS 错误）→ 修复 → 同口径复测对比。截图 _shots/A-*.png。

### 前后对比
| 指标 | 前 | 后 |
|---|---|---|
| 标签列宽 | 36/24/40/34/39/52/130 混乱 | **132px 全站统一** |
| 输入框宽 | 同卡 1064/1042/1054 不齐 | 等宽占满 |
| 描述长度 | 最长 97 字内部话术 | 全部 ≤52 字（五段精简 zh/en） |
| 溢出/错误 | — | 14 分区零溢出零错误 |
...
## 批EE：看板视觉重设计——截图自查驱动（2026-09-22，b5392c8）

### 方法
用户要求「你自己截图看看」：服务器铸超管 token → Playwright localStorage 注入直登生产 → 桌面首屏/告警中心/趋势卡（locator 元素级）+ 移动端 390×844 逐区截图 → 按真实视觉问题改 → 重部署复截核对。滚动容器是内层 `.content`，fullPage 截不到折叠下方（元素级截图绕开）。

### 视觉诊断（截图实证）
1. 开场两条细棒卡片堆叠：hero 64px + 孤行 strip 40px 两张卡贴着，碎、弱
2. hero 中部大片死白（1520 宽下三大数与 chips 之间空一截）
3. 守护概览 2×2 盒中盒：卡里嵌四个带边框小盒，boxy、与左表密度不搭

### 变更（纯前端，Dashboard.vue）
| 项 | 前 | 后 |
|---|---|---|
| KPI 结构 | hero 独立卡 + kpi-zone 卡两张贴着 | **合并一张命令卡**：今日=状态头(tint)+三大数+次级条；非今日=三大主卡+次级条 |
| 布局序 | hero 整行→kpi(60%)/守护(40%)→告警→账户→趋势 | **命令卡整行→账户(60%)/守护待办(40%)→告警整行→趋势整行** |
| 守护概览 | 2×2 边框小盒 | 一行四格分隔线式（移动端 2×2 分隔线重排） |
| hero 数字 | 26px/gap26 | 28px/gap34，tint 底色只染顶部区块 |

### 坑
- CF 边缘缓存：部署后 8s 截图仍旧 bundle（旧 hero 全宽贴 strip 假象「已合并」）→ probe 实测 computed gridColumn 才定案；重部署 20s 后新布局确认
- mint token 必须 `PYTHONPATH=/opt/toveads/backend`（脚本在 /tmp，sys.path 不含 app）
- 截图→Read 返回 CDN URL→`mcp analyze_image` 才能真「看」（首截到登录页=token 空文件，立刻暴露）
...
## 批PP：目标URL/域名批量导入 + 列表化单条管理（ListManager，2026-09-25，79e2f72+补）
