...
const cfZoneDns = ref({})          // {zone_id: records[]} 展开行懒加载
const cfExpanded = ref('')          // 当前展开 DNS 的 zone id
...
const onboardZone = async () => {
  ...
  if (!d) return
  cfOnboarding.value = true
  try {
    cfOnboardResult.value = await POST('/cf-console/zones', { domain: d })
    ElMessage.success(t('settings.cfOnboardOk', { d }))
    await loadCfOverview()
  } catch (e) { ElMessage.error(e.message || t('common.opFail')) }
...
<template>
  ...
  <div class="anchor-strip">
    ...
    <div v-if="isSuper && activeSection==='sec-porkbun'" id="sec-porkbun" class="card">
      ...
      <div class="form-l"><label>{{ t('settings.rgFee') }}</label>
        <el-radio-group :model-value="rgFeeForm.mode" @update:model-value="v => rgFeeForm.mode = v">
          <el-radio-button value="fixed">{{ t('settings.rgFeeFixed') }}</el-radio-button>
          <el-radio-button value="percent">{{ t('settings.rgFeePercent') }}</el-radio-button>
        </el-radio-group>
      </div>
      <template v-if="rgFeeForm.mode === 'fixed'">
        <div class="form-l"><label>{{ t('settings.rgFeeFixedAmt') }}</label><input v-model="rgFeeForm.fixed" class="input" type="number" min="0" step="0.5" :placeholder="t('settings.rgFeePh', { v: rgCfg.fee_rules?.fixed ?? 5 })" /></div>
      </template>
      <template v-else>
        <div class="form-l"><label>{{ t('settings.rgFeeRate') }}</label><input v-model="rgFeeForm.rate" class="input" type="number" min="0" max="100" step="0.5" :placeholder="t('settings.rgFeeRatePh', { v: rgCfg.fee_rules?.rate ?? 5 })" /></div>
...
/* 锚点导航 */
.anchor-strip{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:12px 0 14px;position:sticky;top:-24px;z-index:20;background:var(--bg2);border-bottom:1px solid var(--bd);margin-bottom:14px}
...
@media (max-width: 768px) { .anchor-strip { top: -12px } }
...
.cf-dns-name { min-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap }
