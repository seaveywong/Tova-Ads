...
// 部署抽屉 + 进度弹窗（2026-09-26 组件化抽出到 components/DeployDrawer.vue +
// JobProgressDialog.vue，与广告管理器共用；本页只持 ref 与提交后回调）
const deployDrawer = ref(null)
const jobProgress = ref(null)
const onDeploySubmitted = async (jobId) => { await load(); loadJobs(); jobProgress.value?.open(jobId) }
const openDeploy = (tpl) => deployDrawer.value?.open(tpl)
// 落地页
...
const preflight = async (tpl) => {
  ...
  try {
    ...
    let accs = []
    try { accs = (await GET('/fb/accounts')).filter(a => (a.platform || 'fb') === wantPlat) } catch {}
    ...
    deployDrawer.value?.showPreflight(r)
  } catch (e) { showError(e, t('launch.preflightFail')) }
  preflighting.value = false
}
// ── 部署任务展示/重试共享逻辑（2026-09-26 组件化：useLaunchJobs 与 JobProgressDialog 共用）──
const {
  ensureAccNames, accName, accNameOrTail, statusText, jobText,
  jobElapsed, fmtDur, isItemNote, itemErrDisplay, itemErrTitle,
  itemCodeText, subcodeCount, itemBadgeCls, adsUrl, adsLinkLabel,
} = useLaunchJobs()
</script>
