...
export default {
  zh: {
    ...
    plSentinel: '哨兵', plEmergency: '紧急暂停', plRule: '规则止损', plEmpty: '暂无暂停记录（哨兵/紧急暂停/规则止损的动作会在这里留痕）',
    ...
    rt: { bleed_abs: '空耗止损（消耗无转化）', cpa_exceed: 'CPA 超标', consecutive_bad: '连续恶化', click_no_conv: '点击无转化', reach_no_conv: '覆盖无转化', low_ctr_no_conv: '低 CTR 无转化', budget_burn_fast: '瞬烧制止（增量）', slow_scale: '缓慢扩量（转化优→加预算）', roas_scale: 'ROAS 扩量（ROAS 达标→加预算）' },
    param: { spend_gte: '消耗≥', cpa_target: '目标CPA', ratio: '超标倍数', days: '连续天数', clicks_gte: '点击≥', reach_gte: '覆盖≥', ctr_lte: 'CTR≤', delta_gte: '增量≥', conv_gte: '转化≥', cpa_good: '优秀倍数', roas_gte: 'ROAS≥', step_pct: '每次加', cap_usd: '日预算上限' },
    ...
    human: {
      ...
      slow_scale: '转化≥{conv} 且 CPA≤目标×{ratio} → 日预算 +{pct}%（上限 ${cap}/日，24h 一次）', roas_scale: '转化≥{conv} 且 ROAS≥{roas} → 日预算 +{pct}%（上限 ${cap}/日，24h 一次）',
    },
    inspectScaled: '扩量 {n}',
  ...
  en: {
    ...
    plSentinel: 'Sentinel', plEmergency: 'Emergency', plRule: 'Rule', plEmpty: 'No pause records yet (sentinel / emergency / rule actions will be listed here)',
    ...
    rt: { bleed_abs: 'Bleed stop-loss (spend with no conversion)', cpa_exceed: 'CPA over target', consecutive_bad: 'Consecutive decline', click_no_conv: 'Clicks without conversion', reach_no_conv: 'Reach without conversion', low_ctr_no_conv: 'Low CTR without conversion', budget_burn_fast: 'Budget burn check (delta)', slow_scale: 'Slow scale-up (good CPA → raise budget)', roas_scale: 'ROAS scale-up (ROAS met → raise budget)' },
    param: { spend_gte: 'Spend ≥', cpa_target: 'Target CPA', ratio: 'Over-target ratio', days: 'Consecutive days', clicks_gte: 'Clicks ≥', reach_gte: 'Reach ≥', ctr_lte: 'CTR ≤', delta_gte: 'Delta ≥', conv_gte: 'Conversions ≥', cpa_good: 'Efficiency ratio', roas_gte: 'ROAS ≥', step_pct: 'Raise by', cap_usd: 'Daily budget cap' },
    ...
    human: {
      ...
      slow_scale: 'Conversions ≥ {conv} and CPA ≤ target×{ratio} → daily budget +{pct}% (cap ${cap}/day, once per 24h)', roas_scale: 'Conversions ≥ {conv} and ROAS ≥ {roas} → daily budget +{pct}% (cap ${cap}/day, once per 24h)',
    },
