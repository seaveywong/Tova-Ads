...
1. **Spark Ads MVP 是否做？** 倾向：跳过，Phase 2。（TK 独有，需达人授权链路）
2. **Advanced Matching MVP 是否做？** 倾向：跳过，Phase 2。（需表单采集 PII，合规成本）
3. **FB 是否补 CAPI？** 倾向：TK 接入时顺手把 FB CAPI 也补上（统一双发去重架构），但可独立决策。
4. **平台切换器默认值？** 倾向：默认"全部"（跨平台汇总），用户可切单平台。
