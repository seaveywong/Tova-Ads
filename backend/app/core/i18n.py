...
DATA = {
    ...
    # ── landing.py：自检 checks[].detail（2026-09-23 UI 体检 P0-1 补 i18n）──
    "landing.scDetailDomainUnreachable": {"zh": "不可达: {e}", "en": "Unreachable: {e}"},
    "landing.scDetailWorkerNoResponse": {"zh": "无响应: {e}", "en": "No response: {e}"},
    "landing.scDetailPixelRedirect": {"zh": "redirect 模式（无像素，正常）", "en": "redirect mode (no pixel, normal)"},
    "landing.scDetailPixelSample": {"zh": "（以广告 {ad} 为样本）", "en": " (sampled from ad {ad})"},
    "landing.scDetailPixelCount": {"zh": "{n} 个{samp}：{lst}", "en": "{n}{samp}: {lst}"},
    "landing.scDetailPixelNone": {"zh": "display 未解析到像素（页面不会 fire 转化；有意不带像素可忽略）", "en": "display mode with no pixel resolved (page won't fire conversions; ignore if intentional)"},
    "landing.scDetailConvNone": {"zh": "无像素（无需转化事件）", "en": "no pixel (no conversion events needed)"},
    "landing.scDetailConvFire": {"zh": "CTA 点击 fire：", "en": "CTA click fires: "},
    "landing.scDetailConvEmpty": {"zh": "已配像素但转化事件为空：CTA 点击不会向 FB 上报转化（优化 Purchase 的系列将计 0 转化）", "en": "pixel set but conversion events empty: CTA clicks won't report conversions to FB (Purchase-optimized campaigns count 0 conversions)"},
    "landing.scDetailTargetNone": {"zh": "未配置目标 URL", "en": "no target URL configured"},
    "landing.scDetailTargetUnreachable": {"zh": "不可达: {e}", "en": "unreachable: {e}"},
    "landing.scDetailProtectionOn": {"zh": "已开 · {n} 条规则", "en": "on · {n} rule(s)"},
    "landing.scDetailProtectionEmpty": {"zh": "已开但无规则", "en": "on but no rules"},
    "landing.scDetailProtectionOff": {"zh": "未开启", "en": "off"},
    "landing.scDetailFbSubcodeBlocked": {"zh": "{n}/{m} 个子码被封：{lst}", "en": "{n}/{m} subcodes blocked: {lst}"},
    "landing.scDetailFbSubcodeOk": {"zh": "{n} 个子码全部正常", "en": "{n} subcodes all OK"},
    "landing.scDetailPreviewOn": {"zh": "已启用（审核/测试完记得关）", "en": "enabled (turn off after review/testing)"},
    "landing.scDetailPreviewOff": {"zh": "未启用", "en": "disabled"},
    ...
    "fb.scrapeFailed": {"zh": "FB 爬取失败（防护挡爬虫/SSL/DNS）：{e}", "en": "FB scrape failed (protection blocks crawler/SSL/DNS): {e}"},
    "fb.scrapeError": {"zh": "FB 返回异常：{e}", "en": "FB returned error: {e}"},
